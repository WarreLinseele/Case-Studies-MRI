...
# Selecting 24-parameter motion model and ACompCor and global signal
"""
...
def run_first_level(sub_id: str, paths: dict, output_dir: str, overwrite: bool = False) -> dict:
    ...
    beta_paths = {name: os.path.join(fl_dir, f"{sub_id}_{name}_beta.nii.gz") for name in contrasts}
...
# Compute per-subject mean FD and merge into phenotype dataframe
 ...
 # Prevents motion artifacts inflating neuro-BOLD correlations
...
# with FDR-BH corrections across all tests (one correction per ROI to keep tests within the same brain region grouped; adjust as needed)
def roi_regression(roi_df: pd.DataFrame, pheno: pd.DataFrame, output_dir: str) -> pd.DataFrame:
    ...
    for col in roi_cols:
        ...
        try:
            ...
            results.append({"roi_contrast": col, "n": len(df),
